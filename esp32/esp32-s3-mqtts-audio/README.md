...
| VCC  | 3.3V | 供电 |
| GND  | GND  | 地线 |
| **L/R** | **GND** | **选择左声道(必须接地或接VCC)** |
| SCK  | GPIO7 | BCLK时钟 |
| WS   | GPIO8 | 字选择(LRCK) |
| SD   | GPIO9 | 数据输入 |

**重要**: INMP441的L/R引脚必须接地(GND)或接VCC，不能悬空！
- 接GND → 输出左声道数据
- 接VCC → 输出右声道数据
- 悬空 → 会输出立体声交错数据，导致VAD误触发
...
**症状**: 不说话时能量值异常高(>10000)，VAD持续触发录音

**原因**: INMP441的L/R引脚悬空，导致输出立体声交错数据

**解决方案**:
1. **硬件修复**(推荐): 将INMP441的L/R引脚接地(GND)
2. **软件修复**: 已在代码中添加 `slot_mask = I2S_STD_SLOT_LEFT` 强制读取左声道

**验证方法**: 查看日志中的能量值
- 正常: 安静时200-1000，说话时2000-5000
- 异常: 安静时>10000，采样值呈现交替模式
